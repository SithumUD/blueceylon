...
// MOCK DATA matching backend PublicBusinessController response for Guides
const mockGuides = [
  {
    id: "guide_1",
    name: "Nimal Fernando",
  ...
  },
  {
    ...
    name: "Kumara Silva",
    businessType: "TOUR_GUIDE",
    sltdaRegistrationNumber: "K-4091",
    contactEmail: "kumara.tours@yahoo.com",
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#4A5A62] dark:text-[#A9BCC2] mt-1">
            Manage approved tour guide listings. (Maps to <code className="text-xs bg-gray-200 dark:bg-gray-800 px-1 rounded">GET /api/v1/catalog/public/businesses?type=TOUR_GUIDE</code>)
          </p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#0F252E] rounded-[2rem] border border-[#E4E9EA] dark:border-[#20353D] shadow-sm overflow-hidden flex flex-col">
        <div className="p-6 border-b border-[#E4E9EA] dark:border-[#20353D] flex items-center justify-between">
          <div className="relative max-w-sm w-full">
            ...
            <input 
              type="text" 
              placeholder="Search by guide name or SLTDA ID..."
              value={searchTerm}
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-[#F4F6F8] dark:bg-[#15323D] text-[#4A5A62] dark:text-[#A9BCC2] text-[10px] uppercase font-bold tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4">Guide Type</th>
            ...
            <tbody className="divide-y divide-[#E4E9EA] dark:divide-[#20353D]">
              {mockGuides.map((guide) => (
                <tr key={guide.id} className="hover:bg-gray-50 dark:hover:bg-[#15323D]/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-bold text-[#0E1B22] dark:text-[#EAF2F4]">{guide.name}</div>
                    <div className="text-[10px] text-[#4A5A62] font-mono">{guide.sltdaRegistrationNumber}</div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 font-medium text-[#0E1B22] dark:text-[#EAF2F4]">
                    {guide.vehicleType}
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-xs text-[#0E1B22] dark:text-[#EAF2F4] font-semibold">{guide.contactEmail}</div>
                    <div className="text-[10px] text-[#4A5A62]">{guide.contactPhone}</div>
                  </td>
