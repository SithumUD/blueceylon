...
export default function ToursPage() {
  ...
  // Collect tours from guides (simulated mapping)
  const guideTours = MOCK_GUIDES.map((g) => ({
    id: `g-tour-${g.id}`,
    title: `${g.name} — Private Excursion & Wildlife Tour`,
    description: g.bio,
    duration: "1 Day",
    price: g.dailyRateUSD,
    pricingUnit: "PER_PERSON" as const,
    highlights: g.specialties,
    includedTransport: g.vehicle.type,
    image: g.coverImage,
    providerName: g.name,
    providerId: g.id,
    providerType: "GUIDE",
    location: "Island-wide",
    sltdaVerified: g.sltdaVerified,
  }));

  const allTours = [...hotelTours, ...guideTours].filter((t) => {
    if (filters.city !== "ALL" && t.location.toLowerCase() !== "island-wide" && t.location.toLowerCase() !== filters.city.toLowerCase()) return false;
            ...
            {allTours.map((tour, idx) => (
              <div key={idx} className="group bg-white dark:bg-[#0F252E] rounded-[1.5rem] border border-[#E4E9EA] dark:border-[#20353D] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col">
                <div className="relative aspect-[16/9] overflow-hidden bg-gray-100 dark:bg-gray-800">
                  <img src={tour.image || "https://images.unsplash.com/photo-1534177616072-ef7dc120449d"} alt={tour.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  <div className="absolute top-3 right-3 bg-white/90 dark:bg-black/80 backdrop-blur-md px-2.5 py-1 rounded-full text-[10px] font-bold text-[#0E1B22] dark:text-white flex items-center gap-1 shadow-sm">
                    <Clock className="w-3 h-3 text-[#008080] dark:text-[#3FCFC0]" /> {tour.duration}
                  </div>
                  {tour.sltdaVerified && (
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {tour.highlights?.slice(0, 3).map((hl: string, i: number) => (
                      <span key={i} className="text-[10px] font-semibold bg-gray-50 dark:bg-[#15323D] text-[#4A5A62] dark:text-[#A9BCC2] px-2 py-1 rounded-md flex items-center gap-1 border border-[#E4E9EA] dark:border-[#20353D]">
                        <CheckCircle2 className="w-3 h-3 text-[#008080]" /> {hl}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="mt-auto pt-5 border-t border-[#E4E9EA] dark:border-[#20353D] flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-baseline gap-1">
                        ...
                        <span className="text-xs text-[#9AAAB0]">/pp</span>
                      </div>
                    </div>
